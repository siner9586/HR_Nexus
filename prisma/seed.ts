...
async function resetDemoData() {
  ...
  await prisma.billingEvent.deleteMany({
    where: { OR: [{ stripeEventId: { startsWith: "evt_demo_" } }, { stripeEventId: { startsWith: "evt_mock_" } }] },
  });
...
async function seedPlans() {
  const plans: Array<{
  ...
  }> = [
    {
      code: "FREE",
      name: "免费版",
      description: "适合小团队试用，覆盖员工档案、组织和基础审批。",
      priceMonthly: 0,
      priceYearly: 0,
      maxEmployees: 20,
      maxStorageGB: 1,
      includedAiCredits: 0,
      features: ["员工档案", "组织架构", "基础审批", "合同提醒"],
      sortOrder: 1,
    },
    {
      code: "STANDARD",
      name: "标准版",
      description: "适合成长型企业，提供 HR 日常管理闭环。",
      priceMonthly: 399,
      priceYearly: 3999,
      maxEmployees: 100,
      maxStorageGB: 20,
      includedAiCredits: 50,
      features: ["员工", "组织", "合同", "请假", "审批", "基础考勤", "基础看板"],
      sortOrder: 2,
    },
    {
      code: "PROFESSIONAL",
      name: "专业版",
      description: "适合完整 HR 数字化，加入薪酬、招聘、绩效和培训。",
      priceMonthly: 999,
      priceYearly: 9999,
      maxEmployees: 500,
      maxStorageGB: 100,
      includedAiCredits: 300,
      features: ["标准版全部", "薪资", "工资条", "社保公积金", "招聘", "绩效", "培训", "数据分析"],
      sortOrder: 3,
    },
    {
      code: "ENTERPRISE",
      name: "企业版",
      description: "面向集团和多组织企业，提供审计增强和 API 扩展。",
      priceMonthly: 2999,
      priceYearly: 29999,
      maxEmployees: 3000,
      maxStorageGB: 500,
      includedAiCredits: 1000,
      features: ["专业版全部", "多公司", "多成本中心", "集团 BI", "API", "SSO 预留", "审计增强"],
      sortOrder: 4,
    },
    {
      code: "PRIVATE_DEPLOYMENT",
      name: "私有化部署",
      description: "年度授权、私有部署、定制开发和运维支持。",
      priceMonthly: 0,
      priceYearly: 0,
      maxEmployees: null,
      maxStorageGB: 9999,
      includedAiCredits: 0,
      features: ["私有部署", "定制开发", "年度运维", "专属支持"],
      sortOrder: 5,
      isPublic: false,
    },
  ];

  const result = new Map<TenantPlan, string>();
...
async function seedTenant(input: {
  ...
  const tenant = await prisma.tenant.create({
    data: {
      ...
      subscriptionEndsAt: addDays(now, 330),
      stripeCustomerId: `cus_mock_${input.slug}`,
      stripeSubscriptionId: `sub_mock_${input.slug}`,
      settings: {
        create: {
          payrollEnabled: input.planCode !== "FREE",
          recruitmentEnabled: ["PROFESSIONAL", "ENTERPRISE", "PRIVATE_DEPLOYMENT"].includes(input.planCode),
          performanceEnabled: ["PROFESSIONAL", "ENTERPRISE", "PRIVATE_DEPLOYMENT"].includes(input.planCode),
          trainingEnabled: ["PROFESSIONAL", "ENTERPRISE", "PRIVATE_DEPLOYMENT"].includes(input.planCode),
          aiEnabled: input.planCode === "ENTERPRISE",
        },
      },
    },
  });
  ...
  const planId = input.planIds.get(input.planCode)!;
  const subscription = await prisma.subscription.create({
    data: {
      ...
      stripeCustomerId: `cus_mock_${input.slug}`,
      stripeSubscriptionId: `sub_mock_${input.slug}`,
      stripePriceId: `price_mock_${input.planCode.toLowerCase()}_monthly`,
      status: "MOCK",
      interval: "month",
      seats: input.employeeCount,
      currentPeriodStart: addDays(now, -20),
      currentPeriodEnd: addDays(now, 10),
      amount: input.planCode === "PROFESSIONAL" ? 999 : 2999,
      currency: "CNY",
    },
  });
  ...
  for (let i = 0; i < 3; i += 1) {
    await prisma.invoice.create({
      data: {
        ...
        stripeInvoiceId: `in_mock_${input.slug}_${i}`,
        amountDue: input.planCode === "PROFESSIONAL" ? 999 : 2999,
        amountPaid: input.planCode === "PROFESSIONAL" ? 999 : 2999,
        status: "paid",
        periodStart: addDays(now, -90 + i * 30),
        periodEnd: addDays(now, -60 + i * 30),
        hostedInvoiceUrl: `https://billing.stripe.com/mock/${input.slug}/${i}`,
        invoicePdf: `https://billing.stripe.com/mock/${input.slug}/${i}.pdf`,
      },
    });
  }
  ...
  for (const type of [
    ...
    await prisma.billingEvent.create({
      data: {
        tenantId: tenant.id,
        stripeEventId: `evt_demo_${input.slug}_${type}`,
        type,
        payload: { tenantId: tenant.id, type, mock: true },
        processedAt: now,
      },
    });
  }
  ...
  for (let i = 0; i < 30; i += 1) {
    await prisma.notification.create({
      data: {
        ...
        title: sample(["合同到期提醒", "审批待办", "工资条发布", "转正提醒", "入职任务提醒", "套餐到期提醒"], i),
        content: "这是一条 HR Nexus 演示通知。",
        type: sample(["CONTRACT", "APPROVAL", "PAYSLIP", "REGULARIZATION", "ONBOARDING", "BILLING"], i),
  ...
  const actions: readonly AuditAction[] = ["LOGIN", "VIEW", "CREATE", "UPDATE", "EXPORT", "DOWNLOAD", "APPROVE", "REJECT", "PUBLISH", "SENSITIVE_VIEW", "BILLING_CHECKOUT", "BILLING_WEBHOOK"];
  for (let i = 0; i < 100; i += 1) {
    await prisma.auditLog.create({
      data: {
        ...
        module: sample(["auth", "employees", "payroll", "contracts", "billing", "approvals"], i),
        targetType: sample(["Employee", "SalaryBatch", "Contract", "Subscription"], i),
